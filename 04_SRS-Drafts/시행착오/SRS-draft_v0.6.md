...
| ID | 기술적 제약 (Constraints) |
|---|---|
| **C-TEC-004** | 핵심 가치인 '데이터 보안'을 지키기 위해 외부 LLM API(OpenAI 등)를 사용하지 않고, **독립된 Private Cloud 인프라 내에 오픈소스 sLLM(Gemma)을 자체 구축(Self-Hosted)**하여 운영한다.<br>*(단, sLLM 도입으로 인한 생성 품질 저하 리스크는 **RL 피드백 루프 기반 지속적 파인튜닝 파이프라인**을 통해 방어하며, 실무자(C1)의 피드백이 모델 성능 향상으로 직결되는 플라이휠(Flywheel) 구조를 구축하여 극복한다.)* |
...
| REQ-FUNC-022 | 사용자의 승인/반려/수정 행동 데이터(edit_history)가 자체 구축 sLLM 성능 향상을 위한 **RL 피드백 루프 및 지속적 파인튜닝 파이프라인**의 입력으로 전달되어 품질 향상 플라이휠(Flywheel) 구조를 형성해야 한다. | PRD §4.1 (F5) | Should | **Given** 사용자가 초안을 수정한 후 승인할 때, **When** edit_history가 저장되면, **Then** 해당 이력이 RL 파이프라인으로 500ms 이내에 전달되어 모델 파인튜닝 데이터셋으로 적재되어야 한다. |
