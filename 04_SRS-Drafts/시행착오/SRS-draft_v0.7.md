...
| **F5** | HITL 승인 대시보드 | Should-Have, 프롬프트 개선용 Few-shot 로깅 포함 |
...
| **Vercel AI SDK** | Next.js 환경에서 AI 모델 연동 및 스트리밍을 단일화하는 프레임워크 |
...
| **C-TEC-007** | **(신규 추가)** 모든 데이터 파이프라인(Webhook, DB, LLM 호출)은 **Next.js (App Router)** 기반 단일 풀스택 프레임워크 내에서 구현하여 1인 바이브코딩 복잡도를 최소화한다. |

#### 1.5.2 시스템 외부 (보안 및 AI 통합)

| ID | 기술적 제약 (Constraints) |
|---|---|
| **C-TEC-004** | 자체 구축 복잡도를 제거하고 '완전 바이브코딩' 생산성을 확보하기 위해, **보안 인증(SOC2)을 획득하고 고객 데이터를 학습에 사용하지 않는 엔터프라이즈 API (Azure OpenAI 또는 AWS Bedrock)**를 도입한다. |
| **C-TEC-005** | 서드파티 앱 연동(Slack, Jira) 시에는 고객사 보안 정책을 준수하기 위해 반드시 **최소 읽기 권한(Read-only)**만을 요구한다. |
| **C-TEC-006** | 시맨틱 융합 엔진(Semantic Fusion Engine)은 **Vercel AI SDK를 통한 외부 엔터프라이즈 API 호출 구조**에 종속되며, 외부 API 장애 시 지수 백오프 로직으로 대응하여 시스템 안정성을 확보한다. |
...
시스템의 전체적인 모듈 구성 및 내외부 인터페이스 간의 데이터 흐름은 다음과 같습니다. Vercel AI SDK를 활용하여 Next.js 풀스택 환경에서 엔터프라이즈 LLM API를 호출하는 단일화된 구조를 취합니다.
...
flowchart TD
    ...
    subgraph CorpBrain_Core [CorpBrain Core System / Next.js Full-Stack]
        WH[Webhook Handler]
...
| REQ-FUNC-015 | 정규식 패턴 매칭에 추가하여, 엔터프라이즈 LLM 프롬프트 엔지니어링을 통한 자체 PII 필터링(Zero-shot)을 적용하여 고유명사 기반 기밀 정보를 사전 탐지하여야 한다. (초기 MVP에서는 자체 NER 모델 서빙 생략) | PRD §4.1 (F4) | Must | **Given** 정규식으로 탐지되지 않는 고유명사 기밀이 포함된 텍스트가 존재할 때, **When** 필터링 프롬프트가 적용되면, **Then** 해당 기밀이 추가로 탐지되어 마스킹되어야 한다. |
...
| REQ-FUNC-022 | 사용자의 승인/반려/수정 행동 데이터(edit_history)는 향후 시맨틱 융합 엔진의 성능 향상을 위한 **Few-shot 프롬프트 튜닝용 데이터셋 확보 목적**으로 단순 DB에 로깅되어야 한다. | PRD §4.1 (F5) | Should | **Given** 사용자가 초안을 수정한 후 승인할 때, **When** edit_history가 저장되면, **Then** 해당 수정 이력이 DB에 적재되어 향후 프롬프트 엔지니어링 고도화 자료로 활용되어야 한다. |
...
| REQ-NF-005 | 수정 이력(edit_history) DB 비동기 적재 지연은 ≤ 500ms여야 한다. | edit_history 트랜잭션 처리 시간 | Datadog APM | PRD §5.1 |
...
| ID | 요구사항 | 측정 기준 | 측정 도구 | Source |
|---|---|---|---|---|
| REQ-NF-021 | 1인당 발생하는 LLM 엔터프라이즈 API(Token) 사용 비용은 최대 월 $3.00 이하로 유지하여야 한다. Starter 플랜 $12 기준 최소 75% 마진을 방어한다. | API 벤더사 과금 일별 집계, 인당 환산 비용 | Grafana 대시보드 | PRD §5.5 |
...
| TC-015 | REQ-FUNC-022, 023 | Integration | 수정 이력 로깅 및 프롬프트 개선 피드백 전송 검증 |
...
| `edit_history` | JSONB | NULLABLE | 사용자 수정 이력 (Few-shot 프롬프트 개선 데이터용, REQ-FUNC-022) |
...
sequenceDiagram
    ...
    Note over Parser, Regex: Phase 1: 이중 마스킹 처리 (정규식 + 커스텀 키워드)
    Parser->>Regex: 원시 텍스트 전달
    Regex->>Regex: PII 패턴 매칭 (주민번호, 계좌번호 등)
    Regex->>Dict: 워크스페이스별 커스텀 키워드 조회
    Dict-->>Regex: 커스텀 패턴 목록 반환
    Regex->>Regex: 커스텀 키워드 탐지
    
    alt 마스킹 대상 발견
        Regex->>Regex: 해당 토큰을 *** 치환
    end
    
    alt 마스킹 처리 지연 > 5초 OR 실패
        Regex->>Regex: Critical Alert 발생
        Note over Regex: PagerDuty 알림 + 파이프라인 즉시 중단 (REQ-NF-017)
    end
    
    Regex->>DB: RAW_EVENT 저장 (마스킹 완료 텍스트 + deeplink_url NOT NULL)
    Regex->>LLM: 마스킹된 텍스트만 전송 (원본 절대 미전송)
    LLM-->>Regex: 시맨틱 병합 결과 반환
```

#### 6.3.2 HITL 승인 → 위키 발행 → 수정 이력 로깅 상세
...
sequenceDiagram
    ...
    alt 승인 (수정 포함)
        ...
        API->>DB: approval_status = approved, edit_history 비동기 저장
        ...
        API->>DB: approval_status = rejected, 반려 이력 로깅
    end
```
...
> **End of Document**
> Document ID: SRS-001 | Revision: v0.6 | Date: 2026-05-15 | Standard: ISO/IEC/IEEE 29148:2018
