...
- **Source PRD:** CorpBrain MVP — PRD v0.4 (2026-05-14)
...
| ID | 제약사항 | 근거 |
|---|---|---|
| CON-01 | Kafka 비동기 큐 아키텍처 채택 필수 | ADR-01: 다중 채널 동시 수집 시 Deadlock 방지 및 데이터 유실률 0% SLO 달성 |
...
| ID | 문서명 | 설명 |
|---|---|---|
| REF-01 | CorpBrain MVP PRD v0.4 | 본 SRS의 원천 요구사항 문서 |
...
|---|---|---|---|
| `POST` | `/webhook/slack` | Slack 이벤트 수신 및 Kafka 적재 | F1 |
| `POST` | `/webhook/jira` | Jira 이벤트 수신 및 Kafka 적재 | F1 |
...
sequenceDiagram
    ...
    WH->>Kafka: 메시지 발행 (비동기 적재)
    WH-->>Slack: 200 OK (p95 ≤ 2초)

    Note over Kafka, DB: Phase 2 — 정규화 및 마스킹 (F4)
    Kafka->>Parser: 메시지 소비 및 파싱
    Parser->>PG: 원시 텍스트 전달
...
| REQ-FUNC-003 | 수신된 Webhook 이벤트는 Kafka 메시지 큐를 경유하여 비동기로 적재되어야 한다. 동기식 폴링은 사용하지 않는다. | CB-STORY-001, F1 | Must | **Given** Webhook Handler가 이벤트를 수신한 상태에서, **When** 이벤트를 Kafka에 발행하면, **Then** Kafka Consumer가 해당 메시지를 소비하여 DB에 RAW_EVENT로 적재하여야 한다. Kafka 메시지 수 vs. DB 적재 레코드 수 일치율 = 100%. |
...
| REQ-FUNC-007 | 시스템은 시맨틱 임베딩 기반 의미론적 병합을 수행하여 유사 발화를 하나의 문맥으로 통합하여야 한다. | CB-STORY-001, F2 | Must | **Given** 시간순 정렬된 RAW_EVENT 집합이 존재할 때, **When** Semantic Fusion Engine이 가동되면, **Then** 의미론적으로 유사한 발화가 하나의 구조화된 섹션으로 병합되어야 한다. 문맥 충돌/오류 발생률 < 5%. |
...
| ID | 요구사항 | 측정 기준 | 측정 도구 | Source |
|---|---|---|---|---|
| REQ-NF-007 | API Rate Limit 대비 백오프 및 큐 시스템을 통해 데이터 유실률 0%를 달성하여야 한다. | Kafka 메시지 수 vs. DB 적재 레코드 수 자동 대조 | 주 1회 자동 대조 스크립트 | PRD §5.2 |
...
| REQ-NF-014 | 전송 구간은 TLS 1.3을 적용하여야 한다. | TLS 버전 검증 | SSL Labs 테스트 | PRD §5.3 |
...
| REQ-NF-024 | Kafka 메시지 큐는 수평 파티셔닝을 통해 처리량을 선형적으로 확장할 수 있어야 한다. | 파티션 증설 후 처리량 비례 증가 여부 | ADR-01 |
...
|---|---|---|---|
| TC-001 | REQ-FUNC-001 | Integration | Slack Webhook 수신 및 Kafka 발행 E2E 테스트 |
| TC-002 | REQ-FUNC-002 | Integration | Jira Webhook 수신 및 Kafka 발행 E2E 테스트 |
| TC-003 | REQ-FUNC-003 | Integration | Kafka → DB 적재 일치율 검증 |
...
|---|---|---|---|---|
| EXP-001 | 딥링크 100% 매핑 시 초안 승인율 20%↑ | Closed Beta 30명, 대조군(딥링크 無) vs. 실험군(딥링크 有), 2주 | p-value < 0.05, 승인율 20%p 상회 | REQ-FUNC-010~013 |
...
> **End of Document**
> Document ID: SRS-001 | Revision: 1.0 | Date: 2026-05-14 | Standard: ISO/IEC/IEEE 29148:2018
