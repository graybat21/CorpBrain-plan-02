...
| ID | 제약사항 | 근거 |
|---|---|---|
| CON-01 | 경량 메시지 큐(Redis 등) 비동기 아키텍처 채택 필수 | PRD §7.2 (ADR-01): 초기 인프라 오버헤드 최소화 및 데이터 유실률 0% SLO 달성 |
| CON-02 | 파서(Parser) 원천 기술은 외부 조달 | PRD §7.2 (ADR-02): 자체 개발 시 MVP 일정 3개월+ 지연 리스크 |
| CON-03 | 서드파티 API 연동 시 Read-only 권한만 요구 | 고객사 보안 정책 준수 (PRD §5.3) |
| CON-04 | 자체 구축한 오픈소스 LLM(Gemma) 인프라 가용성에 종속 | 내부 독립 클라우드 GPU 인스턴스 장애 시 융합 엔진 중단 |
| CON-05 | 고객사 Admin의 서드파티 앱 권한 허용 필수 | 온보딩 전제 조건 (PRD §7.3) |

#### 1.2.4 Assumptions (가정)

| ID | 가정 | 상태 |
|---|---|---|
| ASM-01 | 타겟 스타트업의 1일 평균 메시지 생성량은 인당 500건 이하 | 가설 (PRD §7.3) |
| ASM-02 | 실무자(C1)는 기존 툴 UI 변화 없이 대시보드 알림 확인에 거부감 없음 | 가설 (PRD §7.3) |
| ASM-03 | Slack Events API 및 Jira Webhooks는 안정적으로 가용함 | 전제 (PRD §7.3) |
...
| REQ-NF-005 | RL 피드백 루프 전송 지연은 ≤ 500ms여야 한다. | edit_history 저장 후 RL 파이프라인 도달 시간 | 내부 이벤트 로그 | PRD §5.1 |
...
> **End of Document**
> Document ID: SRS-001 | Revision: v0.4 | Date: 2026-05-14 | Standard: ISO/IEC/IEEE 29148:2018
