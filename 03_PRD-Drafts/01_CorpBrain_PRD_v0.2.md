...
* **3.1 Must-Have (MVP 필수 - Sprint 1 內 구현 목표로 범위 한정):**
  * **F1. Zero-UI 백그라운드 자동 수집기:** 사용자 개입 없는 백그라운드 데이터 수집. *(스프린트 1에서는 Slack 연동 우선 구현, Jira는 스프린트 2로 이관)*
...
* **Story ID:** `CB-STORY-001`
  ...
  * **Acceptance Criteria:**
    ...
    * **[실패 케이스 AC]** [Given] 써드파티 API Rate Limit 초과로 수집 지연이 발생할 때, [When] 메시지 큐 재시도(Backoff) 횟수가 임계치를 초과하면, [Then] 대시보드에 즉시 경고 알림을 띄우고 수집 중단 전까지의 데이터만으로 초안을 생성하여 무한 대기를 방지해야 한다. (데이터 증발률 0%)
...
* **5.5 비용 (Cost):**
  * 1인당 월간 LLM 토큰 소모 비용(API Cost)은 최대 $3.00 이하로 유지 보장 (Starter 플랜 $12 기준 최소 75% 마진 방어).
  * 비용 모니터링 대시보드 연동 및 월 한도 한도액 80% 초과 시 Alert 발생.
