...
- **F5. HITL (Human-in-the-Loop) 승인 대시보드 (Should-Have):**
    - **상세 기능:** 시맨틱 엔진이 생성한 초안을 사용자가 '승인/수정/반려'할 수 있는 직관적 인터페이스를 제공합니다. 사용자의 승인/반려 행동 데이터는 향후 시맨틱 융합 엔진의 프롬프트 튜닝용(Few-shot) 데이터셋으로 활용되어 초안 품질을 점진적으로 개선하는 '데이터 플라이휠'을 형성합니다.
...
* **Story ID:** `CB-STORY-003`
  ...
  * **Acceptance Criteria:**
    ...
    * [Given] 사용자가 대시보드에서 딥링크 대조 후 AI 초안을 수동으로 교정할 때, [When] 수정을 완료하면, [Then] 교정 내역이 로깅되어 프롬프트 개선 데이터셋으로 전달되어야 한다. (비동기 적재 지연 ≤ 500ms)
...
erDiagram
    ...
    SEMANTIC_DRAFT {
        ...
        json edit_history "사용자 수정 이력 (프롬프트 개선용)"
...
**설계 근거:** `RAW_EVENT`에 `source_deeplink_url` 필드를 필수(NOT NULL)로 배치함으로써 Trust-Anchor(F3)의 100% 딥링크 매핑을 데이터베이스 레벨에서 강제합니다. `MASKING_RULE`을 Workspace별로 분리하여, 고객사마다 고유한 기밀 키워드 사전을 독립적으로 관리할 수 있는 멀티테넌트 구조를 확보합니다. `SEMANTIC_DRAFT`의 `edit_history`(JSON)는 향후 시맨틱 융합 엔진의 프롬프트 개선용 데이터로 활용됩니다.
...
| **Processing** | Enterprise LLM API (Azure/AWS) | 시맨틱 병합 및 초안 생성 | 토큰 비용 $3/인/월 상한 준수, 마스킹 후 전송 필수 |
...
* **7.2 주요 구조 설계 결정 (ADR - Architecture Decision Records):**
  ...
  * **ADR-03. 단일 풀스택 프레임워크 (Next.js App Router) 및 Enterprise LLM 도입:** 1인 IT 개발 체제에서 극심한 MLOps 유지보수 부하를 제거하고 '바이브코딩' 생산성을 극대화하기 위해 자체 LLM을 배제하고 Vercel AI SDK를 활용한 단일화된 아키텍처를 채택함.
* **7.3 가정 (Assumptions):**
