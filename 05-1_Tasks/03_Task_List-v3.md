# CorpBrain MVP — 개발 태스크 리스트 (Merged Final - Refined)

> **생성일:** 2026-07-16
> **버전:** v3.0 (최종 정제본 - 오버스펙 타협 및 중복 통폐합 반영)

---

## 최적화 및 정제 내역 (v2.0 -> v3.0)

1. **중복 통폐합 (태스크 3건 축소)**
   - 로컬 REST API 셋업을 프로젝트 초기화(`INFRA-001`)로 병합
   - PII 소켓 연결 전 보장 검증을 PII 차단 로직(`LLM-BE-06`)의 성공 조건으로 병합
   - 워크스페이스 50개 보존 한도를 영속성 로드(`WS-BE-07`) 스펙으로 병합
2. **비현실적 오버스펙 타협 (구현 난이도 H -> M으로 하향)**
   - **Ollama 설치:** 완벽한 백그라운드 무인 설치(Silent Install) 대신 사용자 설치 유도 + 연결 헬스체크 플로우로 변경 (`LLM-BE-07`)
   - **딥링크 매핑:** 환각(Hallucination)에 취약한 문장 단위 매핑을 포기하고, 파일 단위 출처 표시로 완화 (`DL-BE-01`)
   - **위키 부분 갱신:** LLM 문맥이 깨지기 쉬운 부분 Merge 대신, 변경된 폴더 단위의 전체 재생성(Regenerate)으로 안정성 확보 (`WA-BE-04`)
   - **유휴 감지:** 백신 오진 위험이 높은 키보드 훅킹(Hooking) 대신 안전한 주기적 일괄 처리(Batch) 모드로 대체 (`WA-BE-05`)
...
|---------|-----------------|---------------|------------|--------|
| INFRA-001 | 프로젝트 구조 및 로컬 REST API 스캐폴딩 (React + Python IPC 바인딩) | §3.2, §3.3, §3.6 | None | M |
...
| INFRA-005 | PyInstaller 단일 `.exe` 패키징 파이프라인 구축 | §1.2 S-01, CON-02 | INFRA-001 | H |
| INFRA-006 | 앱 데이터 경로 격리 (`%LocalAppData%\CorpBrain`) | §4.2 REQ-NF-004 | INFRA-001 | L |
| INFRA-007 | 로그 시스템 구축 (로테이션: 50MB / 30일) | §4.2 REQ-NF-014 | INFRA-001 | L |
| INFRA-008 | 예외 처리 프레임워크 (MAX_PATH 260자, 권한 거부 → Skip & Log) | §4.2 REQ-NF-007, CON-04 | INFRA-007 | M |
...
| Task ID | Feature (기능명) | 관련 SRS 섹션 | 선행 태스크 | 복잡도 |
|---------|-----------------|---------------|------------|--------|
| WS-BE-01 | WorkspaceManager — 워크스페이스 CRUD (생성/조회/삭제) | §4.1.1 REQ-FUNC-001~002, §6.1 #1~4 | INFRA-001, INFRA-004 | M |
...
| WS-BE-07 | 워크스페이스 영속성 (앱 재시작 후 히스토리 로드 및 최대 50개 보존) | §4.1.1 REQ-FUNC-002, §4.2 REQ-NF-008, 013 | WS-BE-01 | L |
...
| Task ID | Feature (기능명) | 관련 SRS 섹션 | 선행 태스크 | 복잡도 |
|---------|-----------------|---------------|------------|--------|
| LLM-BE-01 | LLMRouter — Option A/B 라우팅 디스패처 공통 인터페이스 | §4.1.2 REQ-FUNC-007, §6.4 LLMRouter | INFRA-001 | H |
| LLM-BE-02 | Option A: Cloud LLM API 클라이언트 (OpenAI/Anthropic 통합) | §4.1.2 REQ-FUNC-007, §3.1 | LLM-BE-01 | M |
| LLM-BE-03 | Option B: Ollama 로컬 LLM 클라이언트 연동 | §4.1.2 REQ-FUNC-007, §3.1 | LLM-BE-01 | M |
...
| LLM-BE-06 | PIIFilter — 무결성 검증 및 전송 차단 (소켓 연결 전 메모리상 검증 보장) | §4.1.2 REQ-FUNC-009, §4.2 REQ-NF-006 | LLM-BE-04, LLM-BE-05 | M |
| LLM-BE-07 | Ollama 설치 가이드 제공 및 데몬 연결 상태 확인 API (온보딩 타협) | §4.1.2 REQ-FUNC-010, §6.3.2 | None | M |
...
| LLM-FE-02 | Ollama 수동 설치 유도 및 연결 확인 프로그레스 바 UI | §4.1.2 REQ-FUNC-010 | LLM-BE-07, LLM-UI-01 | M |
...
| Task ID | Feature (기능명) | 관련 SRS 섹션 | 선행 태스크 | 복잡도 |
|---------|-----------------|---------------|------------|--------|
| DL-BE-01 | DeepLinkBridge — 위키 문서 생성 시 출처 파일(File 레벨) 단위 매핑 추가 | §4.1.5 REQ-FUNC-020, §6.4 | ANA-BE-04 | M |
...
| Task ID | Feature (기능명) | 관련 SRS 섹션 | 선행 태스크 | 복잡도 |
|---------|-----------------|---------------|------------|--------|
| DL-FE-01 | 위키 내 딥링크 아이콘/배지 렌더링 (출처 표시) | §4.1.5 REQ-FUNC-020 | DL-BE-01, DL-UI-01, ANA-FE-02 | M |
...
| WA-BE-04 | WatcherDaemon — 변경분 재파싱 및 변경된 폴더 단위의 위키 재생성(Regenerate) | §4.1.6 REQ-FUNC-025, §3.4.2 | WA-BE-03, ANA-BE-04 | M |
| WA-BE-05 | WatcherDaemon — 주기적 일괄 처리 모드 (N분 단위 모아서 배치 처리) | §4.1.6 REQ-FUNC-026, §6.3.5 | WA-BE-02 | M |
...
| WA-FE-03 | 위키 자동 갱신 Toast 알림 + 탭 리렌더링 | §4.1.6 REQ-FUNC-025 | WA-BE-04, WA-UI-01 | M |
...
| NF-002 | Performance | 파일 스캔 p95 <5,000ms (1,000개 기준) 최적화 | §4.2 REQ-NF-001 | WS-BE-02 | M |
| NF-003 | Performance | 심층 분석 100파일(평균 5KB) 300초 이내 처리량 보장 | §4.2 REQ-NF-003 | ANA-BE-04 | M |
| NF-004 | Reliability | Graceful Degradation (LLM 미연결 시 기존 위키·딥링크·대시보드 정상 동작) | §4.2 REQ-NF-010 | LLM-BE-01, ANA-FE-02 | M |
| NF-005 | Reliability | RPO/RTO 보장 (비정상 종료 후 30초 이내 복구, 마지막 DB 커밋 보존) | §4.2 REQ-NF-011 | INFRA-004, WS-BE-07 | M |
| NF-006 | Scalability | 10,000개 파일 전체 파이프라인 (스캔→분석→위키) 동작 검증 | §4.2 REQ-NF-012 | WS-BE-02, ANA-BE-04 | H |
| NF-007 | Maintainability | 사용자 설정(LLM, Watcher) Export/Import (JSON/TOML) | §4.2 REQ-NF-015 | INFRA-004 | L |
| NF-008 | Cost | Option A 파일당 평균 API 비용 산출 및 누적 비용 표시 | §4.2 REQ-NF-016 | LLM-BE-02, STAT-BE-05 | M |
| NF-009 | Monitoring | 내부 Health Metrics 로그 (분석 성공/실패, 평균 소요, Watcher 건수) | §4.2 REQ-NF-017 | INFRA-007 | L |
...
| **M** (2~4일) | 49 | 98~196일 |
| **L** (1일 이하) | 26 | 13~26일 |
| **합계** | **87** | **171~294일 (약 8~14개월, 1인 기준)** |

> 2인 풀타임 기준 약 **4~7개월**, 3인 기준 약 **3~5개월** 예상. 비현실적 오버스펙을 덜어내면서 기존 버전에 비해 **약 1개월 분량의 리스크가 단축**되었습니다.
...
| REQ-FUNC-009 | LLM-BE-06 (NF-002 통합) | ✅ |
...
| REQ-NF-002 | WA-BE-06 | ✅ |
| REQ-NF-003 | NF-003 | ✅ |
| REQ-NF-004 | INFRA-006 | ✅ |
| REQ-NF-005 | NF-001 | ✅ |
| REQ-NF-006 | LLM-BE-06 | ✅ |
| REQ-NF-007 | INFRA-008 | ✅ |
| REQ-NF-008 | WS-BE-07 | ✅ |
| REQ-NF-009 | RN-BE-04 | ✅ |
| REQ-NF-010 | NF-004 | ✅ |
| REQ-NF-011 | NF-005 | ✅ |
